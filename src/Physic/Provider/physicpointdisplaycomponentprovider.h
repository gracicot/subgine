...
{
public:
	PhysicPointDisplayComponentProvider(std::weak_ptr<const PhysicPoint<n>> physicPoint, Vector<freedom(n), double> orientation = Vector<freedom(n), double>(), Vector<n, double> scale = {1});
	PhysicPointDisplayComponentProvider(std::weak_ptr<const PhysicPoint<n>> physicPoint, std::function<Vector<freedom(n), double>()> orientation, Vector<n, double> scale = {1});
