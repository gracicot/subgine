...
{
public:
	PhysicPointDisplayComponentProvider(std::weak_ptr<const PhysicPoint<n>> physicPoint, Vector<freedom(n), double> orientation = Vector<freedom(n), double>(), Vector<n, double> scale = Vector<n, double>());
	PhysicPointDisplayComponentProvider(std::weak_ptr<const PhysicPoint<n>> physicPoint, std::function<Vector<freedom(n), double>()> orientation, Vector<n, double> scale = Vector<n, double>());
	PhysicPointDisplayComponentProvider(std::weak_ptr<const PhysicPoint<n>> physicPoint, Vector<freedom(n), double> orientation, std::function<Vector<n, double>()> scale);
	PhysicPointDisplayComponentProvider(std::weak_ptr<const PhysicPoint<n>> physicPoint, std::function<Vector<freedom(n), double>()> orientation, std::function<Vector<n, double>()> scale);
